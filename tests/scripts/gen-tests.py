...
def gen_basic_tests():
    ...
    for p in FONT_DIR.iterdir():
        if p.suffix in [".ttf", ".otf"]:
            normalized = font_name_to_function(p.name)

            functions = ["glyph_outlines_all_glyphs", "glyph_outlines_single_glyphs"]

            for function in functions:
                function_name = f"{normalized}_{function}"

                test_string += "#[test] "
                test_string += f'fn {function_name}() {{{function}("{p.name}")}}\n'

    with open(Path(BASICS_PATH), "w+") as file:
